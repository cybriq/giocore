...
func getLdFlags(appID string) string {
	...
	ldflags = append(ldflags, fmt.Sprintf("-X github.com/cybriq/giocore/app/internal/log.appID=%s", appID))
	// Pass along all remaining arguments to the app.
	if appArgs := flag.Args()[1:]; len(appArgs) > 0 {
		ldflags = append(ldflags, fmt.Sprintf("-X github.com/cybriq/giocore/app.extraArgs=%s", strings.Join(appArgs, "|")))
	}
	if m := *linkMode; m != "" {
